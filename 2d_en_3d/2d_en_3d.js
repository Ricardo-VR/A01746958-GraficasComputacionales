...
function main() 
{
    ...
    let pac = createPac(gl);

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [1.25, .75, -3.333]);
    mat4.scale(modelViewMatrix, modelViewMatrix, [0.75, 0.75, 1])
    draw(gl, triangle);
    ...
    mat4.scale(modelViewMatrix, modelViewMatrix, [0.75, 0.75, 1])
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.65, -.75, -3.333]);
    mat4.rotateZ(modelViewMatrix, modelViewMatrix, Math.PI/4)
    draw(gl, square);

    mat4.identity(modelViewMatrix);

    mat4.scale(modelViewMatrix, modelViewMatrix, [0.50, 0.50, 1])
    mat4.translate(modelViewMatrix, modelViewMatrix, [2, -1.25, -3.333]);
    mat4.rotateZ(modelViewMatrix, modelViewMatrix, 8 * Math.PI/6)
    draw(gl, pac);
}
...
function createPac(gl) {
    ...
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    let pacman = {buffer:vertexBuffer, vertSize:3, nVerts:numbVerts, primtype:gl.TRIANGLE_FAN};
